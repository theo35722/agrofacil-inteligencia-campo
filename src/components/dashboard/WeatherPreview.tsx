...
export const WeatherPreview = ({
  ...
  const {
    ...
    recommendation,
    forecast
  } = useWeatherProcessor(weatherData as WeatherSourceData | null);
  ...
  return <Link to="/clima" className="block">
        ...
        <Card className={`hover:shadow-md transition-shadow rounded-lg shadow-md ${isMobile ? 'max-w-[90%] mx-auto' : 'w-full'}`}>
          <CardHeader className="pb-2 p-4">
            <CardTitle className="text-agro-green-800 flex justify-between items-center text-lg">
              <span className="py-0 px-0 text-lg">Previsão do Tempo</span>
              <CloudSun className="h-5 w-5 text-agro-blue-500" />
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            {currentWeather && <WeatherPreviewContent currentWeather={{
            ...currentWeather,
            rainChance: todayForecast?.rainChance,
            wind: todayForecast?.wind
          }} locationName={locationName} recommendation={recommendation} showMetrics={true} />}
          </CardContent>
        </Card>
      </div>
    </Link>;
